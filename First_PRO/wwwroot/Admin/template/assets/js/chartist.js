(function ($) {
    ...
    // Function to generate months from January to December
    const months = [
    ...
    const data = {
        ...
        series: [
            Array.from({ length: 12 }, () => Math.floor(Math.random() * 500) + 1) // Random data for y-axis (1 to 500)
        ]
    };
    ...
    const options = {
        axisX: {
            labelInterpolationFnc: function (value, index) {
                return index % 1 === 0 ? value : null; // Show every label
            }
        },
        axisY: {
            labelInterpolationFnc: function (value) {
                return value % 100 === 0 ? value : null; // Show labels every 100 units
            }
        },
        high: 100, // Set the maximum value on the y-axis to 100
        fullWidth: true,
        chartPadding: {
            right: 40
        }
    ...
    var randomData = times(12).map(function () {
        return Math.floor(Math.random() * 500) + 1; // Random data for y-axis (1 to 500)
    });

    var scattererData = {
        labels: months,
        series: times(12).map(function () {
            return randomData;
        })
    };

    if ($('#ct-chart-line-scatterer').length) {
        new Chartist.Line('#ct-chart-line-scatterer', scattererData, options);
    }
